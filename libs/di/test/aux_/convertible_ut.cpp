...
BOOST_AUTO_TEST_CASE(arithmetic_value) {
    const int i = 42;
    BOOST_CHECK_EQUAL(i, static_cast<int>(convertible<int>(i)));
}

BOOST_AUTO_TEST_CASE(arithmetic_with_value_type) {
    const int i = 42;
    BOOST_CHECK_EQUAL(i, static_cast<int>(convertible<named<int> >(i)));
}

BOOST_AUTO_TEST_CASE(string_value) {
    const std::string s = "string";
    BOOST_CHECK_EQUAL(s, static_cast<const std::string&>(convertible<std::string>(s)));
}

BOOST_AUTO_TEST_CASE(string_with_value_type) {
    const std::string s = "string";
    BOOST_CHECK_EQUAL(s, static_cast<const std::string&>(convertible<named<std::string> >(s)));
}

BOOST_AUTO_TEST_CASE(variant_ref) {
    c c_;
    c& c_ref_ = convertible<c>(c_);
    BOOST_CHECK_EQUAL(&c_, &c_ref_);
}

BOOST_AUTO_TEST_CASE(variant_const_ref) {
    c c_;
    const c& const_c_ref_ = convertible<c>(c_);
    BOOST_CHECK_EQUAL(&c_, &const_c_ref_);
}

BOOST_AUTO_TEST_CASE(variant_shared_ptr) {
    shared_ptr<c> c_(new c);
    shared_ptr<c> sp_c = convertible<c>(c_);
...
BOOST_AUTO_TEST_CASE(variant_function) {
    ...
    f_type f_(&c::f);
    BOOST_CHECK_EQUAL(i, static_cast<const f_type&>(convertible<f_type>(f_))());
}

BOOST_AUTO_TEST_CASE(named_int) {
    const int i = 42;
    convertible<named<int> > convertible_(i);

    BOOST_CHECK_EQUAL(i, static_cast<int>(convertible_));
}
...
BOOST_AUTO_TEST_CASE(named_shared_ptr) {
    ...
    BOOST_CHECK((
        *static_cast<shared_ptr<c1_t> >(convertible<c1_t>(c1_))->i
        !=
        *static_cast<shared_ptr<c2_t> >(convertible<c2_t>(c2_))->i
    ));
}
...
BOOST_AUTO_TEST_CASE(context) {
    ...
    BOOST_CHECK((
        static_cast<int>(convertible<int, a>(87))
        !=
        static_cast<int>(convertible<int, b>(42))
    ));

    BOOST_CHECK((
        static_cast<shared_ptr<c> >(convertible<c, a>(c1_))
        !=
        static_cast<shared_ptr<c> >(convertible<c, b>(c2_))
    ));
}

BOOST_AUTO_TEST_CASE(if_shared_ptr) {
    shared_ptr<c0if0> c0_(new c0if0);
    shared_ptr<if0> c1_ = convertible<if0>(c0_);
