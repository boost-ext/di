...
public:
    explicit heat_pump(std::shared_ptr<iheater> heater)
        : heater(heater)
    { }
...
void manual_di() {
    ...
    auto heater = std::shared_ptr<iheater>{
        std::make_shared<electric_heater>()
    };

    // has to be after heater
    auto pump = std::unique_ptr<ipump>{
        std::make_unique<heat_pump>(heater)
    };
