...
BOOST_AUTO_TEST_CASE(braced_initialization) {
    const auto i = 42;
    const auto d = 87.0;
    auto injector = di::make_injector(
        di::bind<int>::to(i)
    ...
    auto object = injector.create<braced>();

    BOOST_CHECK(dynamic_cast<c3if0*>(object.if0_.get()));
    BOOST_CHECK_EQUAL(i, dynamic_cast<c3if0*>(object.if0_.get())->i);
    BOOST_CHECK_EQUAL(i, object.i_);
    BOOST_CHECK_EQUAL(d, object.d_);
}
