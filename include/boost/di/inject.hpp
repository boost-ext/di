...
#define BOOST_DI_GEN_CTOR_IMPL(p, i) BOOST_DI_IF(i)(BOOST_DI_COMMA(),) BOOST_DI_IF(BOOST_DI_IBP(p))(BOOST_DI_EAT p, p)
...
#define BOOST_DI_GEN_TYPE_LIST_IMPL(p, n)  BOOST_DI_IF(n)(BOOST_DI_COMMA(),) BOOST_DI_IF(BOOST_DI_IBP(p))( \
  ...
  , BOOST_DI_IF(BOOST_DI_IS_EMPTY(p))(,const ::boost::di::aux::type<arg##n BOOST_DI_COMMA() ::std::false_type>&))

#if !defined(BOOST_DI_INJECT_TRAITS)
    #define BOOST_DI_INJECT_TRAITS(...) \
        struct BOOST_DI_INJECTOR { \
            BOOST_DI_EVAL(BOOST_DI_REPEAT(BOOST_DI_NARG(__VA_ARGS__), BOOST_DI_GEN_ARGS, __VA_ARGS__)) \
            using type = ::boost::di::aux::type_list<BOOST_DI_EVAL(BOOST_DI_REPEAT(BOOST_DI_NARG(__VA_ARGS__), BOOST_DI_GEN_TYPE_LIST, __VA_ARGS__))>; \
    ...
    #define BOOST_DI_INJECT(type, ...) \
        ...
        type(BOOST_DI_EVAL(BOOST_DI_REPEAT(BOOST_DI_NARG(__VA_ARGS__), BOOST_DI_GEN_CTOR, __VA_ARGS__)))
#endif
