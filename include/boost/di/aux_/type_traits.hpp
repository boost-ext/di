...
#define BOOST_DI_HAS_TYPE_IMPL(name)                            \
    ...
    std::true_type has_##name##_impl(typename T::name*);        \
                                                                \
    template<class T>                                           \
    struct has_##name : decltype(has_##name##_impl<T>(0)) { }
...
#define BOOST_DI_HAS_METHOD_CALL_IMPL(name, f)                                                      \
    struct has_##name##_base {                                                                      \
        void f() { }                                                                                \
                                                                                                    ...
                                                                                                    \
    template<class T>                                                                               \
    std::false_type has_##name##_impl(T*, non_type<void (has_##name##_base::*)(), &T::f>* = 0);     \
                                                                                                    \
    template<class>                                                                                 \
    std::true_type has_##name##_impl(...);                                                          \
                                                                                                    \
    template<class T, typename = void>                                                              \
    struct has_##name : decltype(has_##name##_impl<inherit<T, has_##name##_base>>(0)) { };          \
                                                                                                    \
    template<class T>                                                                               \
    struct has_##name<T, std::enable_if_t<!std::is_class<T>::value>> : std::false_type { }
...
#define BOOST_DI_HAS_METHOD_SIGN_IMPL(name, f)                                                      \
    ...
    std::false_type has_##name##_impl(...);                                                         \
                                                                                                    \
    template<class T, class TSign>                                                                  \
    struct has_##name : decltype(has_##name##_impl<T, TSign>(0)) { }
