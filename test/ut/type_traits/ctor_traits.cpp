...
struct t3 {
    BOOST_DI_INJECT((template<class T1, class T2, class T3>) t3, T1, int, const T2*, (named = n) T3&);
};
...
test templates = [] {
    ...
    static_expect(std::is_same<aux::pair<direct, aux::type_list<_, int, const _*, detail::named_type<decltype(n), _&>>>, ctor_traits__<t3>::type>{});
};
