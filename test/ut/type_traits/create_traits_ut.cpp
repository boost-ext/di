...
BOOST_AUTO_TEST_CASE(create_empty) {
    aux::unique_ptr<empty> empty_(create_traits<policy<false>, empty, empty>());
...
BOOST_AUTO_TEST_CASE(create_ctor) {
    ...
    aux::unique_ptr<ctor> ctor_(create_traits<policy<false>, ctor, ctor, decltype(i), decltype(d)>(i, d));
    BOOST_CHECK(ctor_.get());
}

BOOST_AUTO_TEST_CASE(create_int_value) {
    aux::unique_ptr<int> i(create_traits<policy<false>, int, mpl::int_<42>>());
    BOOST_CHECK_EQUAL(42, *i);
}

BOOST_AUTO_TEST_CASE(create_string_value) {
    aux::unique_ptr<std::string> s(create_traits<policy<false>, std::string, mpl::string<'s'>>());
