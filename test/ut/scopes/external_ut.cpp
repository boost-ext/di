...
BOOST_AUTO_TEST_CASE(from_arithmetic) {
    const int i = 42;
    BOOST_CHECK_EQUAL(i, (external<>::scope<int>(i).create<fake_create_policy>())(type<int>()));
}
...
BOOST_AUTO_TEST_CASE(from_context) {
    ...
    BOOST_CHECK((
        (external<>::scope<int, a>(87).create<fake_create_policy>())(type<int>())
        !=
        (external<>::scope<int, b>(42).create<fake_create_policy>())(type<int>())
    ));
